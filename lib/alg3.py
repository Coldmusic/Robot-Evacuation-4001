...
class Alg3:
	def __init__(self, radius, x_orig, y_orig):
		...
		#Angles
		self.a1 = random.randint(0, 360)
		self.a2 = random.randint(0, 360)

		#First Robot
		self.r1 = random.randint(0, self.r)
		self.x1 = int(self.x0 + self.r1*cos(radians(self.a1)))
		self.y1 = int(self.y0 - self.r1*sin(radians(self.a1)))
	...
	def steps(self):
		if(self.reached == 1):
			...
			self.y_temp = self.y2
			self.point = 0
			s1 = math.sqrt((self.x3-self.x2)**2 + (self.y3-self.y2)**2)
			self.add = (1 / s1) if s1 != 0 else 0
		if(self.reached == 2):
			self.x_temp = self.x1
			self.y_temp = self.y1
			self.point = 0
			s2 = math.sqrt((self.x3-self.x1)**2 + (self.y3-self.y1)**2)
			self.add = (1 / s2) if s2 != 0 else 0
	...
	def circle_steps(self):
		...
		self.point = 0
		self.point1 = 0
		s1 = math.sqrt((self.xt-self.x1)**2 + (self.yt-self.y1)**2)
		self.add = (1 / s1) if s1 != 0 else 0
		s2 = math.sqrt((self.xt-self.x2)**2 + (self.yt-self.y2)**2)
		self.add1 = (1 / s2) if s2 != 0 else 0
